...
// Note: Tag and push commands are delegated to native edgit CLI.
// Only edgitStatus (info) and showEdgitHelp are handled here.
 ...
 * Show edgit help
 *
 * Note: All commands except 'info' and 'status' are delegated to native edgit CLI.
 * This ensures consistent behavior with proper registry integration.
 */
export function showEdgitHelp(): void {
  banners.edgit();
  console.log(`${colors.bold("Commands (delegated to native edgit):")}
...
${colors.bold("Commands (handled by ensemble):")}
  info              Show rich Edgit project info (with banners)
  status            Git status passthrough

${colors.bold("Tag Subcommands:")}
  tag create <name> <ver>  Create immutable version tag
  tag set <name> <env>     Set/move environment tag
  tag list [component]     List tags
  tag show <name>@<tag>    Show tag details
  tag delete <name>@<tag>  Delete a tag

${colors.bold("Examples:")}
  ${colors.accent("edgit tag create extraction v1.0.0")}       ${colors.dim("# Uses component name from registry")}
  ${colors.accent("edgit tag set extraction staging v1.0.0")}  ${colors.dim("# Deploy to staging")}
  ${colors.accent("edgit push --tags --force")}                ${colors.dim("# Push env tags")}
  ${colors.accent("ensemble edgit info")}                      ${colors.dim("# Rich status display")}

${colors.bold("Architecture:")}
  • Native edgit handles registry lookup, prefix inference, validation
  • Ensemble CLI adds rich UI for 'info' command
  • All tag/push operations go through native edgit
